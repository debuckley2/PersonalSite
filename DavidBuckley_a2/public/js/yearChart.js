...
YearChart.prototype.update = function(){
    ...
    // ******* TODO: PART I *******
    self.svg.append("line").attr("x1", 40).attr("y1", 50).attr("x2", self.svgWidth-30).attr("y2", 50)
    .attr("class","lineChart")
    self.xCoordScale = d3.scaleLinear()
        .domain([0,self.electionWinners.length -1])
        .range([40,self.svgWidth - 30])
    ...
    .attr("cx",(d,i)=>{
        return self.xCoordScale(i);
    })
    .attr("cy", 50)
    .on("click",(d)=>{
        d3.csv(`data/election-results-${d.target.__data__.YEAR}.csv`)
            .then(function(yearData) {
                ...
                self.electoralVoteChart.update(yearData, self.colorScale);
                self.votePercentageChart.update(yearData);
        ...
        self.svg.selectAll("circle")
            ...
            .attr("cy", 50)
            .attr("r", 12.5);
        d3.select(d.target).attr("class", "highlighted")
    })
    .attr("r", 12.5);
